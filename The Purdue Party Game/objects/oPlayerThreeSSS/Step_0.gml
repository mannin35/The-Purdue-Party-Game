	...
	if(walksp>0) {
		...
		if (y <= 16) {
			if !(over) {
				...
				show_debug_message("player 3 pos = " + global.minigameResults[2]);
				oSSSControl.pos++;
			...
			if((dist_to_fin<250 && global.CPUSettings[2]==1) || (dist_to_fin<400 && global.CPUSettings[2]==2)) {
			    direct_path = true;	
			}
			if(!direct_path) {
...
if (!over && !hit && !grab && !grabbed) {
	...
	if (left_input) { // if left or right are pressed
		if (alarm_get(0) < 0) {
			...
			//image_xscale = -1.6;
			//hsp = -walksp;
			if ((x - player_two_SSS.x < 64 && x - player_two_SSS.x >= 0 && y == player_two_SSS.y) || (x - player_one_SSS.x < 64 && x - player_one_SSS.x >= 0 && y == player_one_SSS.y) || (x - player_four_SSS.x < 64 && x - player_four_SSS.x >= 0 && y == player_four_SSS.y)) {
				hsp = 0;
			} else {
				hsp = -walksp;
			}
		...
		if (alarm_get(0) < 0) {
			...
			//image_xscale = 1.6;
			//vsp = -walksp;
			if ((y - player_two_SSS.y < 64 && y - player_two_SSS.y >= 0 && x == player_two_SSS.x) || (y - player_one_SSS.y < 64 && y - player_one_SSS.y >= 0 && x == player_one_SSS.x) || (y - player_four_SSS.y < 64 && y - player_four_SSS.y >= 0 && x == player_four_SSS.x)) {
				vsp = 0;
			} else {
				vsp = -walksp;	
			} 
...
if ((place_meeting(x + hsp, y, player_two_SSS) || place_meeting(x + hsp, y, player_one_SSS) || place_meeting(x + hsp, y, player_four_SSS)) && !hit) { // player collision horiz
	hsp = 0;
}
if ((place_meeting(x, y + vsp, player_two_SSS) || place_meeting(x, y + vsp, player_one_SSS) || place_meeting(x, y + vsp, player_four_SSS)) && !hit) { // player collision vert
	vsp = 0;
}
